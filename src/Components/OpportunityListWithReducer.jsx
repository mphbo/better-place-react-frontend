...
const useStyles = makeStyles((theme) => ({
 ...
 const initialState = {
  ...
  const SET_OPPORTUNITY_LIST_DATA = "SET_OPPORTUNITY_LIST_DATA";

  const reducer = (state, action) => {
    switch (action.type) {
      case SET_OPPORTUNITY_LIST_DATA:
        return {
          ...state,
          opportunities: action.opportunities,
          usersOpportunities: action.usersOpportunities,
          users: action.users,
...
const OpportunityList = ({
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then((all) => {
        dispatch({
          type: SET_OPPORTUNITY_LIST_DATA,  
          opportunities: all[0].data.opportunities,
          usersOpportunities: all[1].data.usersOpportunities,
          users: all[2].data.users,
          reviews: all[3].data.reviews
        })
      })
